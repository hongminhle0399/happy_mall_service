...
export const productVariantsEntity = pgTable(TABLE_NAME, {
    id: integer().primaryKey().generatedAlwaysAsIdentity(),
    productId: integer('product_id').references(() => productsEntity.id, {
        onDelete: 'cascade',
    }).notNull(),
    inventoryId: integer('inventory_id').references(() => inventoryEntity.id).notNull(),
